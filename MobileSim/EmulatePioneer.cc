  ...
  switch(pkt->getID())
  {
    ...
    */
    ...
    case ArCommands::CONFIG:
       ...
       replyPkt.byte2ToBuf((ArTypes::Byte2)(robotInterface->haveGripper()?1:0)); // have gripper
...
//log("XXX LRF_ENABLE for laser #%d with %d\n", index, intVal);
    ...
    case ArCommands::SIM_CTRL:
    {
      ...
      if(intVal == SIM_CTRL_LOAD_MAP || intVal == SIM_CTRL_MASTER_LOAD_MAP)
      {
        ...
        // XXX TODO XXX robotInterface->loadNewMapAsync(mapfile, &newMapLoadedCB);
        break;
      }
...
ArRobotPacket* SIPGenerator::getPacket()
{
  ...
  if(robotInterface->sonarOpen() && robotInterface->haveSonar())
  {
    ...
    print_debug("SIP: sonar has %d readings. Max per packet is %d.\n", n, params->Sim_MaxSonarReadingsPerSIP);
#endif
...
#ifdef DEBUG_SIP_SONAR_DATA
        print_debug("SIP: Sonar %d => %d (converted from %d) )\n", s, convr, r);
#endif
...
#ifdef DEBUG_SIP_SONAR_DATA
    print_debug("SIP: Will start with sonar reading #%d in next SIP.\n", firstSonarReadingToSend);
#endif
  }
  else
  {
  ...
  pkt.byteToBuf((char)0xFF);   // digital in, used for IR on peoplebot (0 means triggered, 1 means not triggered)
  pkt.byteToBuf((char)0xFF);   // digital out (0 means triggered, 1 means not triggered)
  if(params->BatteryType == 2)
...
bool EmulatePioneer::sendSIMSTAT(ArDeviceConnection *con)
{
  ...
  if(mapLoader.haveMapOriginLLA)
  {
    ...
    double lat, lon, alt;
    mapCoords.convertMap2LLACoords(x, y, mapOrigin.getAltitude(), lat, lon, alt);
    replyPkt.byte4ToBuf((int)(lat*10e6));
    replyPkt.byte4ToBuf((int)(lon*10e6));
    replyPkt.byte4ToBuf((int)(alt*100));
    if(insideBadGPSSector(ArPose(x, y, th)))
    {
