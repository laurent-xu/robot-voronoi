...
RobotInterface* StageRobotFactory::createRobot(const std::string& modelName, const std::string& requestedRobotName)
{
  ...
  {
    stg_print_msg("Robot Factory: Creating robot of type %s...", modelName.c_str());
    model = stg_world_new_model(myWorld, (char*)modelName.c_str(), NULL, NULL);
  }
  else
  {
    stg_print_msg("Robot Factory: Creating robot of type %s with name %s...", modelName.c_str(), requestedRobotName.c_str());
  ...
  {
    stg_print_msg("Robot Factory: New robot will have starting position (fixed) %f,%f,%f.", myStartPos.x, myStartPos.y, myStartPos.a);
  ...
  else if(myGetStartCB)
  {
    ...
    stg_print_msg("Robot Factory: New robot will have starting position (obtained) %f,%f,%f.", myStartPos.x, myStartPos.y, myStartPos.a);
    stg_model_set_pose(model, myStartPos);
  }
  ...
  return new StageInterface(myWorld, model, modelName, name);
}
