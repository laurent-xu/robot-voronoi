...
"and (C) Copyright 2011-2015, 2014 Adept Technology Inc.\n" \
...
//bool EnableCrashRestart = true;	// if also NonInteractive
...
//  MobileSimConfig config;
  ...
  /* Convert Aria map to Stage world, and load it. TODO don't create robot
     * modelste there.  */
  world = create_stage_world(opt.map.c_str(), robotInstanceRequests, robotFactoryRequests, libdir, mobilesim_startplace, opt.start_pos_override_pose_x, opt.start_pos_override_pose_y, opt.start_pos_override_pose_th, opt.world_res/1000.0);
  ...
  */
  ...
  while(stageStat == 0)
  {
    ...
    if(untilStageUpdate < untilClientOutput)
    {
      ...
      //print_debug("stage update took %ld ms", lastStageUpdate.mSecSince());
    }
    else
    {
      ...
      //print_debug("client output took %ld ms", lastClientOutput.mSecSince());
    }
    ...
    if( (stageUpdateDue.mSecTo() > clientInputWindow && clientOutputDue.mSecTo() > clientInputWindow) || lastClientInput.mSecSince() > clientInputMaxFreq )
    {
      ...
      //print_debug("client input took %ld ms", lastClientInput.mSecSince());
    }

    /* Map loader */
    if( (stageUpdateDue.mSecTo() > mapProcessWindow && clientOutputDue.mSecTo() > mapProcessWindow) || lastMapProcess.mSecSince() > mapProcessMaxFreq )
    {
    ...
    mobilesim_start_place_t startplace, double start_override_x, double start_override_y, double start_override_th, double world_res)
{
  ...
  stg_world_t* world = stg_world_create_from_file(worldfile, options.echo_stage_worldfile); 
  if(!world)
  {
    if(options.NonInteractive)
    {
