...
{  
  int err;
  stg_model_t* mod;
  PRINT_DEBUG6("stg_model_create called with world=%p, parent=%p, id=%d, token=\"%s\", base_type_name=\"%s\", instance_type_name=\"%s\"", world, parent, id, token, base_type_name, instance_type_name);
  ...
  if(parent)
    ...
    mod->root = NULL;
  mod->token = strdup(token); // this will be actually be replaced by
			      // model_create_name  
...
void stg_model_print_ex( stg_model_t* mod, stg_bool_t print_children, int indent)
{
  ...
  printf( "%*smodel %d:%d:%s\n", indent, "", mod->world->id, mod->id, mod->token );
...
void stg_model_load( stg_model_t* mod )
{
  ...
  stg_model_load_from_worldfile_section_id(mod, mod->id);
}
