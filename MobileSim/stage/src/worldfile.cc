...
bool CWorldFile::ParseTokenEntity(int entity, int *index, int *line, const char *macro_name)
{
  int i;
  int macro;
  int name;
  ...
  if (macro >= 0)
  {
    ...
    int mindex = this->macros[macro].starttoken;
    int mline = this->macros[macro].line;
    if (!ParseTokenEntity(entity, &mindex, &mline, macro_name?macro_name:GetTokenValue(name)))
      return false;
    entity = nentity;
